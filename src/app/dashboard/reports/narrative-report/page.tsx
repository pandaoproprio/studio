...
function ActionsArray() {
    ...
    const handleImageUpload = (index: number, files: FileList | null) => {
        if (!files) return;
        // Tipamos currentImages como Array<string> para melhor consistência
        const currentImages = (control._getWatch(`actions.${index}.images`) as string[] | undefined) || [];
        
        Array.from(files).forEach(file => {
            const reader = new FileReader();
            reader.onload = (e) => {
                const dataUri = e.target?.result as string;
                // Garantimos que a array seja tipada no setValue
                setValue(`actions.${index}.images`, [...currentImages, dataUri] as any);
            };
    ...
    const removeImage = (actionIndex: number, imageIndex: number) => {
        // Tipamos currentImages como Array<string>
        const currentImages = (control._getWatch(`actions.${actionIndex}.images`) as string[] | undefined) || [];
        
        // CORREÇÃO APLICADA: Tipagem explícita do parâmetro '_' como 'string'.
        const newImages = currentImages.filter((_: string, i) => i !== imageIndex);
        
        setValue(`actions.${actionIndex}.images`, newImages as any);
    };
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <FormItem>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
                                {(control._getWatch(`actions.${index}.images`) || []).map((imgSrc: string, imgIndex: number) => (
